...
export class Scheduler {
  ...
  private async publishScheduledArticles() {
    try {
      const now = new Date()
      this.logger.info(`[Scheduler] Checking for scheduled articles at ${now.toISOString()}`)
      ...
      this.logger.info(`[Scheduler] Found ${scheduledArticles.length} articles ready for publication`)
