...
import { createWebhook, testWebhookFromData, updateWebhook } from "@/lib/actions/webhooks"
import {
  createWebhookSchema,
...
export function useWebhookForm({ projectId, projectSlug, webhook, mode }: UseWebhookFormProps) {
  ...
  const { setValue, watch, getValues, handleSubmit, register, formState: { errors } } = form
  ...
  const onSubmit = (data: CreateWebhookInput) => {
    startTransition(() => {
      ...
      toast.promise(promise, {
        loading: mode === "create" ? "Creating webhook..." : "Updating webhook...",
        success: () => {
  ...
  const handleTest = useCallback(() => {
    ...
    const firstEvent = (formData.events?.[0] as WebhookEvent) ?? "article.published"
