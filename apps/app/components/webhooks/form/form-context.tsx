...
export const useWebhookFormContext = () => {
  ...
  if (!context) {
    throw new Error(
      "useWebhookFormContext must be used within WebhookFormProvider",
    );
  }
  return context;
};
...
export const WebhookFormProvider: FC<WebhookFormProviderProps> = ({
  children,
}) => {
  const [isPending, setIsPending] = useState(false);
  const [handleTest, setHandleTestState] = useState<(() => void) | undefined>(
    undefined,
  );

  const setHandleTest = useCallback(
    (handler: (() => void) | undefined) => setHandleTestState(() => handler),
    [],
  );

  return (
    <WebhookFormContext.Provider
      value={{ isPending, setIsPending, handleTest, setHandleTest }}
    >
      {children}
    </WebhookFormContext.Provider>
  );
