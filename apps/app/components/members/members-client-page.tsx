...
export const MembersClientPage = ({
  ...
  const [invitations, setInvitations] =
    useState<ProjectInvitation[]>(initialInvitations);
  ...
  const canInviteMore =
    limits.maxMembers === -1 || currentMemberCount < limits.maxMembers;
  ...
  const handleRemoveMember = async () => {
    ...
    toast.promise(removeProjectMember(project.id, removeMemberDialog.id), {
      loading: "Removing member...",
      success: () => {
        setMembers((prev) =>
          prev.filter((m) => m.id !== removeMemberDialog.id),
        );
  ...
  const handleChangeRole = async (
    ...
    toast.promise(updateMemberRole(project.id, memberId, newRoleId), {
  ...
  const handleRevokeInvitation = async () => {
    ...
    toast.promise(revokeProjectInvitation(project.id, revokeInviteDialog.id), {
      loading: "Revoking invitation...",
      success: () => {
        setInvitations((prev) =>
          prev.filter((i) => i.id !== revokeInviteDialog.id),
        );
              ...
              <div className="divide-y">
                {members.map((member) => (
                  ...
                  >
                    ...
                    <div className="flex items-center gap-2">
                      <Badge
                        variant={member.role.isOwner ? "default" : "secondary"}
                      >
                        {member.role.name}
                      </Badge>
                  ...
                  return (
                    ...
                    >
                      <div className="flex items-center gap-3">
                        <Avatar className="rounded-sm">
                          <AvatarImage
                            src={`https://avatar.vercel.sh/${invitation.email.toLowerCase().replaceAll(" ", "")}`}
