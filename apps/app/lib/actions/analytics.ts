...
export const getArticleViewsOverTime = async (
  ...
  const metricsMap = new Map<
    string,
    { uniqueVisitors: number; avgTimeOnPage: number }
  >();
...
export const getBatchArticleViewsOverTime = async (
  ...
  const viewsByArticleAndDate = await prisma.$queryRaw<
    Array<{
      articleId: string;
      date: string;
      views: bigint;
  ...
  articleIds.forEach((articleId) => {
    const dataMap = new Map<
      string,
      { views: number; uniqueVisitors: number; avgTimeOnPage: number }
    >();
...
export const getAllProjectAnalytics = async (
  projectId: string,
  articleIds?: string[],
): Promise<AnalyticsDataMultiPeriod> => {
...
export const getProjectAnalytics = async (
  ...
  const [
    ...
    // Scroll funnel data from PageEvent
    ...
    prisma.pageView.groupBy({
      by: ["utmMedium"],
      where: { ...baseWhere, utmMedium: { not: null } },
      _count: { id: true },
      orderBy: { _count: { id: "desc" } },
      take: 10,
    }) as unknown as Promise<
  ...
  const osStats = osStatsData.map((os) => ({
    os: os.os || "Unknown",
    views: os._count.id,
    percentage: Math.round((os._count.id / totalViews) * 100),
  }));
  ...
  for (let i = days - 1; i >= 0; i--) {
    ...
    const data = viewsOverTimeMap.get(dateStr) || {
      views: 0,
      uniqueVisitors: 0,
      avgTimeOnPage: 0,
    };
  ...
  const sdkViews =
    requestSourceStatsData.find((s) => s.requestSource === "sdk")?._count.id ||
    0;
  const directViews =
    requestSourceStatsData.find((s) => s.requestSource === "direct")?._count
      .id || 0;
  const sdkPercentage =
  ...
  // Process scroll funnel data - need to query eventData for milestone values
  // For now, we'll use the total count from scroll_milestone events
  // A more detailed implementation would parse eventData JSON
  const scrollMilestoneCount =
    scrollFunnelData.find((s) => s.eventType === "scroll_milestone")?._count
      .id || 0;

  // Calculate scroll funnel based on average scroll depth from page views
  const avgScrollDepth = avgMetrics._avg.scrollDepth || 0;
  const scrollFunnel = {
    total: totalViews,
    reached25: Math.round(totalViews * Math.min(1, avgScrollDepth / 25)),
    reached50: Math.round(totalViews * Math.min(1, avgScrollDepth / 50)),
    reached75: Math.round(totalViews * Math.min(1, avgScrollDepth / 75)),
    reached90: Math.round(totalViews * Math.min(1, avgScrollDepth / 90)),
    percentages: {
      reached25: Math.min(100, Math.round((avgScrollDepth / 25) * 100)),
      reached50: Math.min(100, Math.round((avgScrollDepth / 50) * 100)),
      reached75: Math.min(100, Math.round((avgScrollDepth / 75) * 100)),
      reached90: Math.min(100, Math.round((avgScrollDepth / 90) * 100)),
    },
  };

  // Process UTM stats
  const utmStats = {
    sources: utmSourcesData.map((s) => ({
      source: s.utmSource || "Unknown",
      views: s._count.id,
      percentage: Math.round((s._count.id / totalViews) * 100),
    })),
    mediums: utmMediumsData.map((m) => ({
      medium: m.utmMedium || "Unknown",
      views: m._count.id,
      percentage: Math.round((m._count.id / totalViews) * 100),
    })),
    campaigns: utmCampaignsData.map((c) => ({
      campaign: c.utmCampaign || "Unknown",
      views: c._count.id,
      percentage: Math.round((c._count.id / totalViews) * 100),
    })),
  };
