...
const SettingsPage = () => {
  ...
  const form = useForm<UpdateProjectSettingsInput>({
    ...
    defaultValues: {
      ...
      allowedOrigins:
        currentProject?.allowedOrigins?.map((origin: string) => ({
  ...
  const hasIconColorChanges =
    dirtyFields.icon || dirtyFields.color || dirtyFields.avatarUrl;
  ...
  const saveSettings = async (
    ...
    toast.promise(updateProjectSettings(currentProject.id, payload), {
        ...
        switch (section) {
          ...
          case "icon-color":
            form.resetField("icon", {
              defaultValue: i(project.icon || "building-2"),
            });
            form.resetField("color", { defaultValue: project.color || "CYAN" });
            form.resetField("avatarUrl", {
              defaultValue: project.avatarUrl || null,
            });
            break;
          case "language":
            form.resetField("defaultLanguage", {
          ...
          case "origins":
            form.resetField("allowedOrigins", {
              defaultValue:
                project.allowedOrigins?.map((origin: string) => ({
                  value: origin.replace("https://", ""),
                })) || [],
            });
            break;
          case "url":
            form.resetField("baseUrl", {
              defaultValue: project.baseUrl || null,
            });
            form.resetField("articleUrlPattern", {
              defaultValue: project.articleUrlPattern || "/blog/{slug}",
            });
            break;
        }
  ...
  const deleteDisabled =
    !canDeleteProject ||
    deleteConfirm.trim() !== currentProject.slug ||
    isDeleting;

  const handleDeleteProject = async () => {
    if (!currentProject) return;
    setIsDeleting(true);
    toast.promise(deleteProject(currentProject.id), {
  ...
  return (
    ...
    >
      <div className="space-y-6">
        ...
        <Card variant="form">
          <CardHeader>
            <FieldLabel htmlFor="slug" className="text-base font-medium">
              Project Slug
            </FieldLabel>
            <FieldDescription className="mt-1">
        ...
        <Card variant="form">
          <CardHeader>
            ...
            <FieldDescription className="mt-1">
              Choose how your project is displayed in the app.
            </FieldDescription>
          ...
          <CardContent>
            <div className="space-y-4">
              {displayType === "icon" ? (
                <div>
                  <FieldLabel className="mb-2 text-sm">Icon & Color</FieldLabel>
                ...
                <div>
                  <AvatarUpload
                    ...
                    onFileSelect={(file) => {
                      ...
                      if (file) {
                        form.setValue("avatarUrl", "pending", {
                          shouldDirty: true,
                        });
          ...
          <CardFooter>
            <p className="text-muted-foreground text-sm">
              Avatar will override icon if uploaded.
            </p>
            <Button
              type="button"
        ...
        <Card variant="form">
          <CardHeader>
            ...
            <FieldDescription className="mt-1">
              The default language for new articles and language variant system.
            </FieldDescription>
          </CardHeader>
          ...
          <CardContent>
            <CompactLanguageSelector
              ...
              disabled={
                currentProject?.subscriptionTier === "STARTER" || isDisabled
              }
              dialog={true}
            />
          ...
          <CardFooter>
            <p className="text-muted-foreground text-sm">
              Affects translations and date formatting.
            </p>
            <Button
              type="button"
        ...
        <Card variant="form">
          <CardHeader>
            ...
            <FieldDescription className="mt-1">
              Domains that can access your project (CORS).
            </FieldDescription>
          </CardHeader>
        ...
        <Card variant="form">
          <CardHeader>
            ...
            <FieldDescription className="mt-1">
              Configure the base URL and pattern for article links in webhooks.
              Leave empty to use custom URLs in templates.
            </FieldDescription>
          </CardHeader>
          ...
          <CardContent>
            <div className="max-w-md space-y-4">
              <div>
                <FieldLabel htmlFor="baseUrl" className="mb-2 text-sm">
                  Base URL
                </FieldLabel>
                <Input
                  id="baseUrl"
              ...
              <div>
                ...
                <FieldDescription className="mt-1">
                  Pattern for article URLs. Must include {"{slug}"}. Examples:
                  /blog/{"{slug}"}, /articles/{"{slug}"}
                </FieldDescription>
                {errors.articleUrlPattern && (
                  <FieldError>{errors.articleUrlPattern.message}</FieldError>
                )}
              </div>
            </div>
          </CardContent>
        ...
        <Card variant="form-danger">
          ...
          <CardContent>
            ...
            {!canDeleteProject && (
              <p className="text-muted-foreground text-sm">
                You need the <Kbd>Delete project</Kbd> permission{" "}
                <Kbd>OWNER</Kbd> to delete this project.
              </p>
            )}
          </CardContent>
