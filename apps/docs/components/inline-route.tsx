...
export function InlineRoute({ method, path, className }: InlineRouteProps) {
  ...
  return (
    ...
    >
      <span className={cn("text-[10px] font-bold uppercase tracking-wide", styles.text)}>{method}</span>
      <span className="text-foreground/80">{path}</span>
    </code>
  )
}

export function InlineRouteLink({ method, path, href, className }: InlineRouteLinkProps) {
  const styles = methodStyles[method]
  ...
  return (
    ...
    >
      <span className={cn("text-[10px] font-bold uppercase tracking-wide", styles.text)}>{method}</span>
      <span className="text-foreground/80 underline-offset-2 group-hover:underline">{path}</span>
    </a>
  )
}
