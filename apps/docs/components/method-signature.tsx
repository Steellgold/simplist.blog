          ...
          <CardContent className="p-0">
            <pre className="p-4 overflow-x-auto">
              <code className="font-mono text-sm">
                <span className={cn(isAsync && "text-purple-600 dark:text-purple-400")}>{isAsync && "async "}</span>
                ...
                {parameters.map((param, index) => (
                  <span key={param.name}>
                    ...
                    <span className="text-muted-foreground">{param.optional ? "?" : ""}</span>
                    <span className="text-muted-foreground">: </span>
                    <span className="text-emerald-600 dark:text-emerald-400">{param.type}</span>
                    {param.defaultValue && (
                      <>
                        <span className="text-muted-foreground"> = </span>
                        <span className="text-amber-600 dark:text-amber-400">{param.defaultValue}</span>
                      </>
                    )}
                    {index < parameters.length - 1 && <span className="text-muted-foreground">, </span>}
                  </span>
                ))}
                <span className="text-foreground">): </span>
                <span className="text-emerald-600 dark:text-emerald-400">{returnType}</span>
              </code>
            </pre>
          </CardContent>
        </Card>
