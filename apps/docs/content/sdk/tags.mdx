---
title: Tags API
description: Manage and retrieve tags with the Simplist SDK
category: SDK Reference
order: 14
icon: Tag
---

# Tags API

The Tags API provides methods to fetch tags and their metadata from your Simplist project.

## Quick Reference

<ApiMethodTable
  methods={[
    { method: "list", description: "List all tags with article counts", returns: "ApiResponse<TagListItem[]>" },
    { method: "get", description: "Get a specific tag by name", returns: "ApiResponse<TagListItem>" },
    { method: "names", description: "Get all tag names", returns: "Promise<string[]>" },
    { method: "popular", description: "Get tags sorted by article count", returns: "Promise<TagListItem[]>" }
  ]}
/>

## list()

Retrieve all tags with their metadata and article counts.

<MethodSignature
  name="client.tags.list"
  parameters={[]}
  returnType="Promise<ApiResponse<TagListItem[]>>"
  async={true}
/>

### Response Type

<TypeTable
  title="TagListItem"
  properties={[
    { name: "id", type: "string", description: "Unique tag identifier", required: true },
    { name: "name", type: "string", description: "Tag name", required: true },
    { name: "icon", type: "string | null", description: "Icon name (e.g., 'code', 'book')" },
    { name: "color", type: "string | null", description: "Hex color code (e.g., '#3B82F6')" },
    { name: "articleCount", type: "number", description: "Number of articles with this tag", required: true },
    { name: "createdAt", type: "string", description: "ISO 8601 creation timestamp", required: true },
    { name: "updatedAt", type: "string", description: "ISO 8601 last update timestamp", required: true }
  ]}
/>

### Example

<CodeBlock language="tsx">
{`
  const { data: tags, meta } = await client.tags.list()

  console.log(\`Total tags: \${meta.total}\`)

  tags.forEach(tag => {
    console.log(\`- \${tag.name}: \${tag.articleCount} articles\`)
    if (tag.color) {
      console.log(\`  Color: \${tag.color}\`)
    }
    if (tag.icon) {
      console.log(\`  Icon: \${tag.icon}\`)
    }
  })
`}
</CodeBlock>

## get()

Retrieve a specific tag by its name.

<MethodSignature
  name="client.tags.get"
  parameters={[
    { name: "name", type: "string", required: true, description: "Tag name to retrieve" }
  ]}
  returnType="Promise<ApiResponse<TagListItem>>"
  async={true}
/>

### Example

<CodeBlock language="tsx">
{`
  const { data: tag } = await client.tags.get('JavaScript')

  console.log(\`Tag: \${tag.name}\`)
  console.log(\`Articles: \${tag.articleCount}\`)
  console.log(\`Color: \${tag.color}\`)

  // Tags with spaces are automatically URL-encoded
  const { data: mlTag } = await client.tags.get('Machine Learning')
`}
</CodeBlock>

## names()

Get an array of all tag names (convenience method).

<MethodSignature
  name="client.tags.names"
  parameters={[]}
  returnType="Promise<string[]>"
  async={true}
/>

### Example

<CodeBlock language="tsx">
{`
  const tagNames = await client.tags.names()

  console.log('Available tags:', tagNames.join(', '))
  // Output: "JavaScript, TypeScript, Tutorial, Advanced"

  // Use for dropdowns or autocomplete
  const tagOptions = tagNames.map(name => ({
    label: name,
    value: name
  }))
`}
</CodeBlock>

## popular()

Get tags sorted by article count (most popular first).

<MethodSignature
  name="client.tags.popular"
  parameters={[
    { name: "limit", type: "number", optional: true, description: "Maximum number of tags to return" }
  ]}
  returnType="Promise<TagListItem[]>"
  async={true}
/>

### Example

<CodeBlock language="tsx">
{`
  // Get top 5 most used tags
  const topTags = await client.tags.popular(5)

  topTags.forEach((tag, i) => {
    console.log(\`\${i + 1}. \${tag.name} (\${tag.articleCount} articles)\`)
  })

  // Get all tags sorted by popularity
  const allPopularTags = await client.tags.popular()
`}
</CodeBlock>

## Using Tags with Articles

Tags can be used to filter articles using the Articles API. See [Articles API](/sdk/articles) for more details.

### Filter by Tags (OR logic)

Get articles with at least one of the specified tags.

<CodeBlock language="tsx">
{`
  // Get all tags first
  const { data: tags } = await client.tags.list()

  // Filter articles by specific tag
  const { data: jsArticles } = await client.articles.list({
    tags: ['JavaScript']
  })

  // Filter by multiple tags (OR logic)
  const { data: webArticles } = await client.articles.list({
    tags: ['JavaScript', 'TypeScript', 'HTML']
  })
  // Returns articles tagged with JavaScript OR TypeScript OR HTML
`}
</CodeBlock>

### Filter by All Tags (AND logic)

Get articles that have all specified tags.

<CodeBlock language="tsx">
{`
  // Articles that are BOTH tutorials AND about JavaScript
  const { data: jsTutorials } = await client.articles.list({
    tagsAll: ['JavaScript', 'Tutorial']
  })
  // Returns only articles tagged with BOTH JavaScript AND Tutorial
`}
</CodeBlock>

### Exclude Tags

Get articles that don't have specific tags.

<CodeBlock language="tsx">
{`
  // Articles without Advanced or Deprecated tags
  const { data: beginnerArticles } = await client.articles.list({
    excludeTags: ['Advanced', 'Deprecated']
  })
`}
</CodeBlock>

### Combining Tag Filters

You can combine multiple tag filters for complex queries.

<CodeBlock language="tsx">
{`
  // Complex filter: JavaScript OR TypeScript tutorials that are NOT advanced
  const { data: beginnerWebTutorials } = await client.articles.list({
    tags: ['JavaScript', 'TypeScript'],     // OR: at least one of these
    tagsAll: ['Tutorial'],                   // AND: must have this tag
    excludeTags: ['Advanced'],               // NOT: must not have this tag
    published: true
  })

  // Example: Get the most popular tags and articles for each
  const popularTags = await client.tags.popular(3)

  for (const tag of popularTags) {
    console.log(\`\\nTag: \${tag.name} (\${tag.articleCount} articles)\`)

    const { data: articles } = await client.articles.list({
      tags: [tag.name],
      limit: 5
    })

    articles.forEach(article => {
      console.log(\`  - \${article.title}\`)
    })
  }
`}
</CodeBlock>

## Common Use Cases

### Tag Cloud Component

<CodeBlock language="tsx">
{`
  import { SimplistClient } from '@simplist/sdk'

  const client = new SimplistClient({ apiKey: process.env.SIMPLIST_API_KEY })

  export async function TagCloud() {
    const { data: tags } = await client.tags.list()

    return (
      <div className="flex flex-wrap gap-2">
        {tags.map(tag => (
          <a
            key={tag.id}
            href={\`/tags/\${tag.name}\`}
            className="px-3 py-1 rounded-full"
            style={{
              backgroundColor: tag.color || '#gray',
              opacity: 0.7 + (tag.articleCount / 20) * 0.3
            }}
          >
            {tag.name} ({tag.articleCount})
          </a>
        ))}
      </div>
    )
  }
`}
</CodeBlock>

### Popular Tags Widget

<CodeBlock language="tsx">
{`
  export async function PopularTags() {
    const popularTags = await client.tags.popular(5)

    return (
      <aside>
        <h3>Popular Tags</h3>
        <ul>
          {popularTags.map(tag => (
            <li key={tag.id}>
              <a href={\`/tags/\${tag.name}\`}>
                {tag.icon && <Icon name={tag.icon} />}
                {tag.name}
                <span>({tag.articleCount})</span>
              </a>
            </li>
          ))}
        </ul>
      </aside>
    )
  }
`}
</CodeBlock>

### Tag-based Navigation

<CodeBlock language="tsx">
{`
  export async function TagPage({ params }: { params: { tag: string } }) {
    const tagName = decodeURIComponent(params.tag)

    // Get tag info
    const { data: tag } = await client.tags.get(tagName)

    // Get articles with this tag
    const { data: articles } = await client.articles.list({
      tags: [tagName],
      published: true
    })

    return (
      <div>
        <h1>
          {tag.icon && <Icon name={tag.icon} />}
          {tag.name}
          <span>({tag.articleCount} articles)</span>
        </h1>

        <ArticleList articles={articles} />
      </div>
    )
  }
`}
</CodeBlock>

## Error Handling

<CodeBlock language="tsx">
{`
  try {
    const { data: tag } = await client.tags.get('NonExistentTag')
  } catch (error) {
    if (error.statusCode === 404) {
      console.error('Tag not found')
    } else {
      console.error('Failed to fetch tag:', error.message)
    }
  }
`}
</CodeBlock>

## See Also

- <InlineRouteLink method="GET" path="/v1/tags" href="/api/get-tags" /> - REST API endpoint for listing tags
- <InlineRouteLink method="GET" path="/v1/tags/:name" href="/api/get-tag" /> - REST API endpoint for getting a tag
- [Articles API](/sdk/articles) - Using tags to filter articles
- [Articles Types](/sdk/types) - TypeScript type definitions
