...
<ApiMethodTable
  methods={[
    { method: "track", description: "Track initial page view", returns: "PageViewResponse" },
    { method: "update", description: "Update engagement metrics", returns: "{ success: boolean }" },
    { method: "getStats", description: "Get analytics statistics", returns: "AnalyticsStats" }
  ]}
/>
...
<TypeTable
  title="PageViewData"
  properties={[
    { name: "slug", type: "string", description: "Article slug to track", required: true },
    ...
    { name: "utmCampaign", type: "string", description: "UTM campaign parameter" },
    { name: "utmTerm", type: "string", description: "UTM term parameter" },
    { name: "utmContent", type: "string", description: "UTM content parameter" },
    { name: "screenWidth", type: "number", description: "Browser viewport width" },
    { name: "screenHeight", type: "number", description: "Browser viewport height" },
    { name: "timeOnPage", type: "number", description: "Time spent on page (seconds)" },
    { name: "scrollDepth", type: "number", description: "Max scroll depth (0-100)" },
    { name: "exitPosition", type: "number", description: "Scroll position when leaving" },
    { name: "bounced", type: "boolean", description: "Whether user bounced" },
    { name: "timestamp", type: "string", description: "Custom timestamp (ISO 8601)" },
...
<TypeTable
  ...
  properties={[
    { name: "timeOnPage", type: "number", description: "Total time on page (seconds)" },
    { name: "scrollDepth", type: "number", description: "Maximum scroll depth reached (0-100)" },
    { name: "exitPosition", type: "number", description: "Scroll position when exiting (0-100)" },
    { name: "bounced", type: "boolean", description: "Whether user bounced (left immediately)" },
    { name: "events", type: "PageEvent[]", description: "Custom events to add" }
  ]}
/>
...
<CodeBlock
  ...
  export function ArticleAnalytics({ slug }: { slug: string }) {
    ...
    const SESSION_KEY = 'simplist_session'
  ...
  items={[
    {
      ...
      answer: "Track page views for content pages (articles, blog posts). Skip tracking for admin pages, API routes, or frequently visited pages like home. Focus on measuring content performance."
    },
    {
      question: "How do I avoid double-counting in React Strict Mode?",
      answer: "Use useRef to track if analytics was already sent. In development, React Strict Mode mounts components twice, so add a ref check to prevent double tracking."
    },
    {
      question: "Should analytics tracking block page rendering?",
      answer: "No! Always track analytics asynchronously. Don't await the track() call in your component render. Use fire-and-forget pattern or background requests."
    }
  ]}
/>
