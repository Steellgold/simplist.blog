...
export const generateSeoMetadata = (
  ...
  const variant =
    lang && article.variants?.[lang] ? article.variants[lang] : article;
  const isVariant = variant !== article;

  const title = variant.title;
  const summarySource = (variant.content || "").substring(0, 4000);
  const description =
    variant.excerpt || `${summarySource.substring(0, 160)}...`;
  const canonicalUrl = baseUrl
    ? `${baseUrl}/${project.slug}/${article.slug}${isVariant ? `?lang=${lang}` : ""}`
    : undefined;
  ...
  const modifiedTime = new Date(
    variant.updatedAt || article.updatedAt,
  ).toISOString();
...
<rss version="2.0" xmlns:content="http://purl.org/rss/1.0/modules/content/" xmlns:atom="http://www.w3.org/2005/Atom">
  ...
  const rssItems = articles
    .map((article) => {
      ...
      const articlePath = customPath
        ? customPath.replace("{slug}", article.slug)
        : article.slug;
...
<urlset xmlns="http://www.sitemaps.org/schemas/sitemap/0.9" xmlns:xhtml="http://www.w3.org/1999/xhtml">`;
  ...
  const projectUrl =
    !customPath || customPath.includes("{slug}")
      ? `
  <url>
...
export const generateHreflangLinks = (
  ...
  // Add main article
  hreflangLinks.push(
    `    <xhtml:link rel="alternate" hreflang="x-default" href="${baseUrlPath}" />`,
  );

  // Add each variant
  Object.keys(article.variants).forEach((lang) => {
    hreflangLinks.push(
      `    <xhtml:link rel="alternate" hreflang="${lang}" href="${baseUrlPath}?lang=${lang}" />`,
    );
  });
