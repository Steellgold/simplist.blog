...
import cronAuth from "@/plugins/cron-auth";
...
const cronRoutes: FastifyPluginAsync = async (fastify) => {
  // Register cron authentication plugin for all routes
  await fastify.register(cronAuth);

  // POST /cron/publish-scheduled - Publish scheduled articles
  fastify.post("/cron/publish-scheduled", async (request, reply) => {
    try {
      const now = new Date();
      fastify.log.info(
        `Starting scheduled article publication check at ${now.toISOString()}`,
      );
      ...
      fastify.log.info(
        `Found ${scheduledArticles.length} articles ready for publication`,
      );
      ...
      for (const article of scheduledArticles) {
        try {
          ...
          await sendWebhookEvent(article.projectId, "article.published", {
            ...
            publishedAt:
              article.scheduledPublishAt?.toISOString() ?? now.toISOString(),
      ...
      fastify.log.info(
        `Cron job completed: ${results.published}/${results.processed} articles published`,
      );
      ...
      fastify.log.error(
        error,
        "Error in scheduled article publication cron job",
      );
  ...
  fastify.post("/cron/expire-invitations", async (request, reply) => {
    try {
      const now = new Date();
      fastify.log.info(
        `Starting invitation expiration check at ${now.toISOString()}`,
      );
      ...
      for (const invitation of expiredInvitations) {
        try {
          ...
          results.expired++;
          fastify.log.info(
            `Expired invitation: ${invitation.email} for project ${invitation.project.name} (${invitation.id})`,
          );
        } catch (error) {
          ...
          fastify.log.error(
            error,
            `Error expiring invitation ${invitation.id}`,
          );
        }
      }

      fastify.log.info(
        `Cron job completed: ${results.expired}/${results.processed} invitations expired`,
      );
  ...
  fastify.post("/cron/process-account-deletions", async (request, reply) => {
    try {
      const now = new Date();
        ...
        try {
          ...
          if (scheduledAt <= now) {
            try {
              ...
              fastify.log.error(
                err,
                `Failed to send deletion confirmation for user ${user.id}`,
              );
            }
          ...
          fastify.log.error(
            error,
            `Error processing deletion for user ${user.id}`,
          );
        }
      }
