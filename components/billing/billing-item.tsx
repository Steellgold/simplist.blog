...
const getStatusLabelAndIcon = (status: string | null) => {
  ...
  switch (status) {
    ...
    default:
      return { label: status.charAt(0).toUpperCase() + status.slice(1), icon: CheckCircle }
  }
}
...
const formatPaymentMethod = (paymentMethod: PaymentMethodInfo | null | undefined) => {
  ...
  return {
    text: paymentMethod.type.charAt(0).toUpperCase() + paymentMethod.type.slice(1).replace("_", " "),
    ...
    <Item variant="muted">
      ...
      <ItemContent>
        <ItemTitle>
          Invoice
          {entry.number && (
            <span className="bg-muted/90 rounded-xs px-1.5 py-0.5 font-mono text-xs text-muted-foreground">
              {entry.number}
            </span>
          )}
        </ItemTitle>
        ...
        <ItemDescription className="flex items-center gap-1">
          <Badge variant={getStatusColor(entry.status)}>
            {(() => {
              const { label, icon: IconComponent } = getStatusLabelAndIcon(entry.status)
              return (
